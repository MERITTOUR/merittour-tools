-- ════════════════════════════════════════════════════════════════
-- 21_aso_prestige.sql — 8인 별장 이름을 「아소 프레스티지」로  (멱등 · 비파괴)
--
--   야마나미 프레스티지  ┐
--   프레스티지 하우스    ┴→  아소 프레스티지
--
-- 왜 두 이름을 다 받나 — 20 을 이미 실행했으면 저장된 행이 「프레스티지 하우스」로
-- 있고, 아직 안 했으면 「야마나미 프레스티지」로 있다. 둘 다 받아 두면 20 을
-- 돌렸든 안 돌렸든, 20 을 나중에 돌리든 결과가 같아진다.
--
-- 20 은 고치지 않았다. 이미 나간 마이그레이션을 손대면 「어디까지 돌렸는지」가
-- 사람마다 달라진다. 20 → 21 순서로 돌리면 「야마나미 프레스티지 → 프레스티지
-- 하우스 → 아소 프레스티지」로 두 번 옮겨 갈 뿐 결과는 같다.
--
...
--    where hotel in ('야마나미 프레스티지','프레스티지 하우스','아소 프레스티지')
--    group by hotel order by hotel;

do $$
declare
  olds text[] := array['야마나미 프레스티지', '프레스티지 하우스'];
  new_name text := '아소 프레스티지';
  i int;
  moved int;
  clashed int;
begin
...
--   select hotel, count(*) from public.mt_block_override
--    where hotel in ('야마나미 프레스티지','프레스티지 하우스') group by hotel;
