-- ════════════════════════════════════════════════════════════════
-- 30_kuju_private_town.sql — 2인 독채 10동 이름을 「쿠쥬 프라이빗 타운」으로  (멱등 · 비파괴)
--
--   야마나미 프라이빗 레지던스  ┐
--   쿠주 프라이빗 레지던스      │
--   쿠주힐즈                    ┼→  쿠쥬 프라이빗 타운
...
--   쿠쥬 타운하우스             ┘
--
-- 2026-09-16 결정(Min) — 「쿠쥬 타운하우스 → 쿠쥬 프라이빗 타운으로 전체 이름 변경」.
-- 영문 KUJU PRIVATE TOWN · 내부 키 kuzu · 사진 폴더 kuju · 엠클릭 상품명 「쿠주힐즈」(aliases) 는 그대로다.
-- 옛 이름 「쿠쥬 타운하우스」는 리조트 마스터 aliases 에 남겨 붙여넣은 표를 계속 맞춘다.
--
-- 왜 다섯 이름을 다 받나 — 20·26·28·29 를 어디까지 돌렸느냐에 따라 저장된 행이 다섯 중 어느 이름으로도
-- 있을 수 있다. 다 받아 두면 앞의 것을 돌렸든 안 돌렸든 결과가 같아진다. (21~23·26~29 와 같은 방식)
--
...
--    where hotel in ('야마나미 프라이빗 레지던스','쿠주 프라이빗 레지던스','쿠주힐즈','쿠주 타운하우스','쿠쥬 타운하우스','쿠쥬 프라이빗 타운')
--    group by hotel order by hotel;

do $$
declare
  olds text[] := array['야마나미 프라이빗 레지던스', '쿠주 프라이빗 레지던스', '쿠주힐즈', '쿠주 타운하우스', '쿠쥬 타운하우스'];
  new_name text := '쿠쥬 프라이빗 타운';
  i int;
  moved int;
  clashed int;
begin
...
--   select hotel, count(*) from public.mt_block_override
--    where hotel in ('야마나미 프라이빗 레지던스','쿠주 프라이빗 레지던스','쿠주힐즈','쿠주 타운하우스','쿠쥬 타운하우스') group by hotel;
