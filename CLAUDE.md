...
- **올리는 곳은 대시보드 「자료 불러오기」 안에도 있다**(`regSaveUpload`). 올리고 보는 것이 갈라져 있으면 매번 페이지를 왕복해야 한다. `tools/register/` 는 업로드 전용 화면으로 남겨 둔다 — 두 곳이 같은 `MT.scanRes`·`MT_STORE.save` 를 쓰므로 숫자가 갈라지지 않는다.
- 「내 화면에서만 열기」는 서버에 안 올리는 비상구다. 그렇게 연 자료는 다른 직원이 못 본다는 것을 버튼 옆에 적어 둔다.
