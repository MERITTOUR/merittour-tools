...
- 과거 ①~⑪ 단일 번호 체계는 폐기됨.
- **등록된 달은 칩으로 늘어놓고, 누르면 바로 불러온다.** 드롭다운이면 몇 달이 올라와 있는지 열어 봐야 알고 고른 뒤 버튼을 또 눌러야 한다. 칩에는 달·올린 사람·시각·팀·인원·블록표 유무를 함께 적고, 지금 열려 있는 달은 켜 둔다.
- **「＋ 새 자료 올리기」는 접지 않는다.** 매번 펼치는 동작이 늘어날 뿐이다.
- **첫 화면은 파일 업로드가 아니라 「자료 불러오기」다.** 출발월만 고르면 서버에서 받아 `analyze()`까지 돈다. 파일 업로드는 `<details>` 안에 접어 두었다 — 서버가 안 될 때만 쓰는 비상구이고, 그렇게 연 자료는 내 화면에만 있다는 것을 화면에 적어 둔다.
