...
```
owner   마스터      — 계정 발급·역할 변경·삭제. mt_has_role 이 무엇을 물어도 참
admin   관리자      — 예전 값. 새로 주지 않는다(마스터와 거의 같다)
manage  관리팀      — 등록소 등록·수정 · 리조트 마스터 · 예약 · 확정서
sales   영업팀      — 위와 같음
air     항공팀      — 위와 같음  ※ 2026-08 부터 읽기 전용이 아니다
```

> `air` 는 원래 읽기 전용이었다. 조회 정책에는 들어 있는데 쓰기 정책에서만
> 빠져 있어, 항공팀 계정으로 열면 입력칸과 버튼은 보이는데 저장만 조용히
> 0행이 됐다. `18_air_same_as_others.sql` 에서 세 팀을 같은 층으로 맞췄다.
