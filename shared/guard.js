/* ════════════════════════════════════════════════════════════════
   ...
   역할(sales/air/…) 자체로는 문 앞에서 막지 않는다. 막는 기준은 **섹션**이다
   (`data-section`). 역할은 그 섹션 권한의 기본값을 정할 뿐이고, 실제로 누가
   무엇을 여는지는 owner 가 `admin/users/` 에서 계정마다 정한다.
   ※ 루트(index.html)가 모두를 `/sales/` 로 보내던 것도 함께 고쳤다 —
     볼 수 있는 허브로 각자 보낸다. 안 그러면 air 계정이 문 앞에서 막힌다.

   사용법: <head> 안에서 본문보다 먼저 한 줄. 깊이는 상관없다.
     <script src="../shared/guard.js" data-section="sales"></script>
     <script src="../../shared/guard.js" data-section="booking"></script>
   data-section 을 빼면 로그인·승인만 확인한다(예전 동작).
...
(function () {
  ...
  /* 이 화면이 어느 섹션인지. 부르는 줄에 적는다 —
       <script src="../shared/guard.js" data-section="booking"></script>
     안 적으면 예전처럼 로그인·승인만 확인한다(섹션으로 막지 않는다).
     한 줄에 같이 두는 이유는 헤더 주석과 같다: 페이지마다 두 줄을 맞춰
     넣게 하면 어느 한 장에서 빠져도 조용히 안 막힌다. */
  var SECTION = (self && self.getAttribute && self.getAttribute('data-section')) || '';
  ...
  /* 허브에서 볼 수 없는 카드는 아예 감춘다. 눌러 들어갔다가 막히는 것보다
     처음부터 안 보이는 편이 낫다 — 「왜 안 되냐」는 문의가 줄어든다.
     대상은 링크에 data-section 을 적어 둔 카드뿐이다(이 파일을 부르는
     <script data-section> 태그는 a 가 아니라 걸리지 않는다). */
  function hideCards(u) {
    if (!window.MT_AUTH || !window.MT_AUTH.canReadArea) return;
    var cards = document.querySelectorAll('a[data-section]');
  ...
  need('MT_SB', 'supabase-config.js')
    ...
    .then(function (u) {
      ...
      /* 섹션 권한. 페이지가 자기 섹션 키를 밝혀 두면(아래 SECTION 참고)
         읽기 권한이 없을 때 여기서 막는다. 들어가서 빈 화면을 보는 것보다
         **무엇이 막혔고 누구에게 요청해야 하는지**를 문 앞에서 읽는 편이 낫다. */
      if (SECTION && window.MT_AUTH.canReadArea && !window.MT_AUTH.canReadArea(u, SECTION)) {
        CURRENT = u;
        var label = (window.MT_AUTH.labelOf && window.MT_AUTH.labelOf(SECTION)) || SECTION;
        card('열 수 없는 화면입니다',
